use std::{
    fmt::Display, marker::PhantomData, path::{Path, PathBuf}
};
    ...
    K: Display,
{
    ...
    pub async fn get(&self, key: &K) -> color_eyre::Result<V> {
        let read = cacache::read(&self.name, key.to_string()).await?;
        Ok(rmp_serde::from_slice(&read)?)
    }

    // Get an item and return an option if it isn't found. This is more akin to a `std::Hashmap`
